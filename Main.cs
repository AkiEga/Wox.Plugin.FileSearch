...
using Wox.Plugin.FileSearch.Everything;
...
namespace Wox.Plugin.FileSearch
{
    public class Main : IPlugin, ISettingProvider, IPluginI18n, IContextMenu, ISavable
    {
        ...
        private List<ContextMenu> GetDefaultContextMenu()
        {
            ...
            ContextMenu copyPathContextMenu = new ContextMenu
            {
                Name = "Copy path",
                Command = "\"{path}\"",
                Argument = " | clip.exe",
            ...
            defaultContextMenus.Add(copyPathContextMenu);
                    ...
                    if (contextMenu.Name == "Copy path")
                    {
                        contextMenus.Add(new Result
                        {
                            ...
                            Action = _ =>
                            {
                                try
                                {
                                    string fullPath = record.FullPath;
                                    System.Windows.Forms.Clipboard.SetDataObject(fullPath, true);
                                }
                                catch
                                {
        ...
        {
            return new FileSearchSettings(_settings);
        }
    }
}
